...
def get_args():
    ...
    parser.add_argument("--data_path", type=str, default="data\\celeba", help="the root folder of dataset")
    ...
    parser.add_argument("--log_path", type=str, default="tensorboard/yolo_celeba")
...
def train(opt):
    ...
    learning_rate_schedule = {"0": 3e-4, "5": 1e-4, "80": 1e-5, "110": 1e-6}
    ...
    training_set = CelebADataset(opt.data_path, opt.train_set, opt.image_size)
    ...
    if torch.cuda.is_available():
        if opt.pre_trained_model_type == "model":
            ...
            state_dict = torch.load(opt.pre_trained_model_path)
            state_dict['stage3_conv2.weight'] = torch.rand((50910, 1024, 1, 1),dtype=torch.float)
    ...
    # optimizer = torch.optim.Adadelta(model.parameters(), lr=3e-3, weight_decay=opt.decay)
    optimizer = torch.optim.SGD(model.parameters(), lr=3e-4, momentum=opt.momentum, weight_decay=opt.decay)
    best_loss = 1e10
    best_epoch = 0
    model.train()
